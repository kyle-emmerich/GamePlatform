...
namespace Engine {
	...
	namespace Reflection {
		...
		struct Class {
			...
			::std::function<::Engine::Instance* (Engine*)> constructor = nullptr;
			::std::function<::std::shared_ptr<::Engine::Instance>(Engine*)> sharedConstructor = nullptr;
		...
		inline bool Class::IsA(std::string_view className) {
			...
			for (uint64_t baseId : publicBaseClasses) {
				Class* baseClass = Engine::Reflection::registry.GetClassById(baseId);
